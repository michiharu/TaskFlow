...
import { calculate, entityToTree, nodeToEntities, setPoint, setTreeSize } from './funcs';
import { FlowEntitySliceState, adapter } from './slice';
import {
  closedEntities,
  flatEntities,
  horizontalEntities,
...
describe('root only', () => {
  ...
  test('setSize', () => {
    expect(setTreeSize(node)).toEqual(nodeWithSize);
  });
  test('setPoint', () => {
    const point: Point = { x: 0, y: 0 };
    const resultWithPoint: FlowNode = { ...nodeWithSize, point };
    expect(setPoint(nodeWithSize, point)).toEqual(resultWithPoint);
  });
});
...
describe('flat entities', () => {
  ...
  const rootTree: Size = {
    width: indent * m * 2 + card.width + m,
    height: card.height * 4 + m * 7,
  };
  ...
  test('setPoint', () => {
    ...
    expect(setPoint(nodeWithSize, rootPoint)).toEqual(resultWithPoint);
  });
});
...
describe('nested entities', () => {
  ...
  const rootTree: Size = {
    width: indent * m * 4 + card.width + m * 3,
    height: card.height * 4 + m * 7,
  };
  const child1Tree: Size = {
    width: indent * m * 3 + card.width + m * 2,
    height: card.height * 3 + m * 5,
  };
  const child2Tree: Size = {
    width: indent * m * 2 + card.width + m,
    height: card.height * 2 + m * 3,
  };
  const nodeWithSize: FlowNode = {
    ...node,
    tree: rootTree,
...
describe('closed entities', () => {
  ...
  test('setPoint', () => {
    ...
    const child1Point: Point = {
      x: stagePadding + indent * m,
      y: stagePadding + card.height + m,
    };
    const child3Point: Point = {
      x: stagePadding + indent * m,
      y: stagePadding + card.height * 2 + m * 2,
    };
    const resultWithPoint: FlowNode = {
      ...nodeWithSize,
      point: rootPoint,
...
describe('horizontal entities', () => {
  ...
  const rootTree: Size = {
    width: card.width * 4 + indent * m + m * 6,
    height: card.height + indent * m * 2 + m * 3,
  };
  const child1Tree: Size = {
    width: card.width * 2 + indent * m + m * 2,
    height: card.height + indent * m + m * 2,
  };
  const child3Tree: Size = {
    width: card.width + m,
    height: card.height + indent * m,
  };
  const nodeWithSize: FlowNode = {
    ...node,
    tree: rootTree,
    children: [
      {
        ...child1,
        tree: child1Tree,
        children: [{ ...child2, tree: noChildOpenTree, children: [] }],
      },
  ...
  test('setPoint', () => {
    const rootPoint: Point = { x: stagePadding, y: stagePadding };
    const child1Point: Point = {
      x: stagePadding + card.width + m,
      y: stagePadding + indent * m,
    };
    const child2Point: Point = {
      x: stagePadding + card.width * 2 + m * 2,
      y: stagePadding + indent * m * 2,
    };
    const child3Point: Point = {
      x: stagePadding + card.width * 3 + indent * m + m * 4,
      y: stagePadding + indent * m,
    };
...
describe('nested closed entities', () => {
  ...
  const rootTree: Size = {
    width: indent * m + card.width + m,
    height: card.height * 2 + m * 2,
  };
  const nodeWithSize: FlowNode = {
    ...node,
    tree: rootTree,
...
describe('setRect', () => {
  ...
  const state: FlowEntitySliceState = {
    ...
    flow: {
      id: '12345678-9abc-4def-9000-000000000123' as UUID,
      title: '',
      rootId: root.id,
    },
    addablePoints: [],
  ...
  const entities: FlowEntity[] = [
    { ...root, flowIndex: 0, tree: rootTree, point: rootPoint },
    {
      ...child1,
      flowIndex: 1,
      tree: card,
  ...
  test('', () => {
    const nextState = produce(state, (draft) => calculate(draft, adapter));
    const array = adapter.getSelectors().selectAll(nextState);
    expect(array).toEqual(entities);
  });
  test('flow is undefined', () => {
    const flowIsUndefined: FlowEntitySliceState = { ...state, flow: undefined };
    expect(() => calculate(flowIsUndefined, adapter)).toThrow();
  });
  test('no root in entities', () => {
    const noRootInEntities = {
      ...state,
      entities: {
        [child1.id]: child1,
        [child2.id]: child2,
    ...
    expect(() => calculate(noRootInEntities, adapter)).toThrow();
  });
});
